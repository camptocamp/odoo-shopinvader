...
class ProductProduct(models.Model):
    ...
    def _shopinvader_bind(self, backend):
        BindingWizard = self.env["shopinvader.variant.binding.wizard"]
        wizard = BindingWizard.create(
            {
    ...
    def _shopinvader_unbind(self, backend):
        to_unbind = self.shopinvader_bind_ids.filtered(
            lambda binding: binding.backend_id == backend
        )
        to_unbind.active = False
    ...
    def _inverse_company_shopinvader_published(self):
        ...
        for rec in self:
            if rec.company_shopinvader_published:
                rec._shopinvader_bind(backend)
            else:
                rec._shopinvader_unbind(backend)
