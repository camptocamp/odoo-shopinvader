...
class ProductSet(models.Model):
    ...
    def get_line_by_product(self, product_id=None, invader_variant_id=None):
        assert product_id or invader_variant_id
        if product_id:
    ...
    def _compute_shopinvader_variant(self):
        for record in self:
            if record.product_id and not record.shopinvader_variant_id:
                ...
                # from the locomotive request and we cannot make sure which lang
                # is going to be used. Let's rely only on the partner until we solve it.
