...
class ProductSet(models.Model):
    ...
    def get_line_by_product(self, product_id=None, invader_variant_id=None):
        # Backward compat
        _logger.info(
            "DEPRECATED `get_line_by_product`. Use `get_lines_by_product`"
        )
        if product_id:
            product_id = [product_id]
        if invader_variant_id:
            invader_variant_id = [invader_variant_id]
        return self.get_lines_by_product(
            product_ids=product_id, invader_variant_ids=invader_variant_id
        )

    def get_lines_by_product(self, product_ids=None, invader_variant_ids=None):
