    ...
    def _inverse_company_shopinvader_published(self):
        ...
        for rec in self:
            if rec.company_shopinvader_published:
                rec.product_variant_ids._shopinvader_bind(backend)
            else:
                rec.product_variant_ids._shopinvader_unbind(backend)

    def _search_shopinvader_published(self):
        return [("company_shopinvader_bind_ids.active", "=", True)]
